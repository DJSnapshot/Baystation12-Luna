...
MapManager/proc/CompileMap(var/map_name as text)
	...
	var/list/all_models = stringsplit(copytext(file_contents, 1, definition_end), "\n")
	var/list/all_levels = stringsplit(copytext(file_contents, definition_end + 2), "\n\n")
	...
	for(var/model in all_models)
		...
		//	e.g. "aaa" = (/turf/space,/frame)
		//	becomes list("aaa" = "/turf/space,/frame")
	...
	for(var/level in all_levels)
		...
		if(!level)
			break
		var/list/split_level = stringsplit(level, "\n")
...
MapManager/proc/ParseModel(var/model_text)
	...
	var/list/objects = stringsplit(model_text, ",/")
	for(var/object in objects)
		...
		if(variable_seperator)
			...
			for(var/variable in stringsplit(copytext(object, variable_seperator + 1, length(object)), "; "))
...
MapManager/proc/ParseVars(var/list/entry, var/list/strings)
	...
	if(findtext(entry, "list("))
		entry = list()
		for(var/variable in stringsplit(copytext(entry, 6, length(entry)), ", "))
